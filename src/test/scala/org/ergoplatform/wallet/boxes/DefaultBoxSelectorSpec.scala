...
class DefaultBoxSelectorSpec extends PropSpec with Matchers {
  ...
  property("returns None when it is impossible to select coins") {
    ...
    select(Seq(uBox).toIterator, box => box.chainStatus.mainChain, 1, Map()) shouldBe None
  ...
  property("properly selects coins - simple case with no assets") {
    ...
    val s4 = select(uBoxes.toIterator, box => box.chainStatus.mainChain, 11, Map())
  ...
  property("properly selects coins - assets w. 1 change box") {
    ...
    val parentTx = ErgoLikeTransaction(IndexedSeq(), IndexedSeq())
    val box1 = ErgoBox(1, TrueLeaf, StartHeight, Seq(Digest32 @@ idToBytes(assetId1) -> 1))
    ...
    select(uBoxes.toIterator, box => box.chainStatus.mainChain, 1, Map(assetId2 -> 1)) shouldBe None
    ...
    select(uBoxes.toIterator, box => box.chainStatus.mainChain, 1, Map(assetId1 -> 11, assetId2 -> 1)) shouldBe None
  }
  ...
  property("properly selects coins - assets w. multiple change boxes") {
    ...
    val box1 = ErgoBox(1, TrueLeaf, StartHeight,
    ...
    val box2 = ErgoBox(10, TrueLeaf, StartHeight,
    ...
    val box3 = ErgoBox(100, TrueLeaf, StartHeight,
