...
class MnemonicSpec
  ...
  it should "pass https://github.com/trezor/python-mnemonic/blob/master/vectors.json" in {
    ...
    forAll(cases) { (entropy, sentence, seed, _) =>
      runChecks(entropy, sentence, passphrase, seed, lang)
    }
  ...
  it should "generate mnemonic for all Entropy configurations" in {
    Seq(128, 160, 192, 224, 256).foreach { e =>
      Try {
        new Mnemonic("japanese", strength = e).generate
      } shouldBe 'success
    }
  }

  it should "return the same mnemonic given the same entropy" in {
    forAll(mnemonicGen, entropyGen) { (mnemonic, ent) =>
      val m1 = mnemonic.toMnemonic(ent)
      val m2 = mnemonic.toMnemonic(ent)
      m1 == m2
    }
  }
